...
export const PublicMenu = ({ defaultSlug }) => {
  ...
  if (loading) return <div className="min-h-screen bg-zinc-950 flex items-center justify-center text-white">...</div>;
  if (!restaurant) return <div className="min-h-screen bg-zinc-950 flex items-center justify-center text-white uppercase text-xs tracking-widest">404 • Restaurante no encontrado</div>;
  ...
  return (
    <div className="min-h-screen bg-zinc-950" style={{ '--primary-color': restaurant.theme_settings?.primary || '#ec4893' }}>
      ...
      <main className="container mx-auto px-4 py-8 space-y-12 pb-24">
        {menuData?.map((section, index) => <MenuSection key={index} section={section} />)}
      </main>
    </div>
  );
};